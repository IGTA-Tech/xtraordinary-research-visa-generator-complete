...
def set_exhibits_from_classifications(classifications: List[Any], numbering_style: str = 'letters'):
    ...
    for i, c in enumerate(classifications):
        ...
        exhibit = ExhibitItem(
            id=f"exhibit_{i}",
            number=number,
            name=c.criterion_name if hasattr(c, 'criterion_name') else c.get('criterion_name', 'Unknown'),
            filename=c.filename if hasattr(c, 'filename') else c.get('filename', ''),
            category=c.criterion_name if hasattr(c, 'criterion_name') else c.get('criterion_name', 'Other'),
            confidence=c.confidence_score if hasattr(c, 'confidence_score') else c.get('confidence_score', 0.5),
            pages=0,
            file_path=getattr(c, 'file_path', None) if hasattr(c, 'file_path') else c.get('file_path'),
            order=i
        )
